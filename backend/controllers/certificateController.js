...
const CERT_PRICE_NGN = 4000;        // Nigeria → ₦4,000 via Paystack
const CERT_PRICE_USD_CENTS = 1500;  // Everywhere else → $15 via Flutterwave
...
export const initialize = async (req, res) => {
  try {
    ...
    // Geo decides the currency/processor: Nigeria → Paystack ₦; elsewhere → Flutterwave $.
    const country = (req.body?.country || 'NG').toUpperCase();
    const isForeign = country !== 'NG';

    // FOREIGN USERS: $15 via Flutterwave
    if (isForeign) {
      if (!FLW_SECRET || FLW_SECRET.startsWith('FLWSECK_TEST-your')) {
        return res.status(503).json({ success: false, message: 'International card payments are not configured yet.' });
      }
      const reference = `CERTFLW-${Date.now()}-${userId}`;
      const usdAmount = CERT_PRICE_USD_CENTS / 100;
      ...
      return res.json({ success: true, free: false, authorization_url: link, reference, processor: 'flutterwave' });
    }
...
export const verifyPayment = async (req, res) => {
  try {
    const { reference, processor } = req.body;
    const userId = req.user.id;
    ...
    // Already issued for this reference? (idempotent)
    const existing = await db.query('SELECT * FROM certificates WHERE payment_reference = $1', [reference]);
    if (existing.rows.length > 0) {
      return res.json({ success: true, certificate: existing.rows[0], already_issued: true });
    }

    const isFlutterwave = processor === 'flutterwave' || reference.startsWith('CERTFLW-');
    let amount; // what was paid, recorded on the certificate
...
function generatePDF(cert, res) {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="StockAcademia-Certificate-${cert.certificate_number}.pdf"`);
  doc.pipe(res);
