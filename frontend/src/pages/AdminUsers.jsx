...
import {
  Users, MessagesSquare, BarChart3, Calendar, GraduationCap, ShieldCheck,
  TrendingUp, Wallet, BookOpen, Sparkles, ArrowRight, AlertCircle
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[...Array(4)].map((_, i) => <div key={i} className="h-32 bg-ink/5 rounded-3xl animate-pulse"/>)}
          </div>
        ) : stats && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <StatCard icon={Users} label="Total users" value={stats.total_users} sub={`+${stats.signups_today} today`} color="bg-bull-100 text-bull-700"/>
            <StatCard icon={Sparkles} label="Premium subscribers" value={stats.premium_subscribers} sub={`${pct(stats.premium_subscribers, stats.total_users)} of users`} color="bg-coral-300/40 text-coral-500"/>
            <StatCard icon={Wallet} label="Revenue this month" value={`₦${(stats.revenue_this_month_kobo / 100).toLocaleString()}`} sub="Premium + bookings" color="bg-sun-100 text-sun-600"/>
            <StatCard icon={BookOpen} label="Lessons completed" value={stats.lessons_completed} sub={`across ${stats.total_users} users`} color="bg-sage-200 text-bull-700"/>
          </div>
        )}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          <SectionCard to="/admin/users" icon={Users} title="User management"
          ...
          <SectionCard to="/admin/forum" icon={MessagesSquare} title="Forum moderation"
            desc={stats ? `${stats.active_posts} active posts · ${stats.active_comments} comments` : 'Remove abusive content, ban users'}
            color="from-coral-400 to-coral-500"/>
          <SectionCard to="/admin/revenue" icon={BarChart3} title="Revenue & analytics"
            desc="Track Premium, bookings, MRR over time"
            color="from-sun-400 to-coral-400"/>
          <SectionCard to="/admin/bookings" icon={GraduationCap} title="Mentorship bookings"
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <ActivityCard title="Recent signups" empty="No signups yet">
            {activity.signups.map((u) => (
              <Link key={u.id} to={`/admin/users/${u.id}`} className="flex items-center justify-between py-2 hover:bg-cream-warm rounded-lg px-2 -mx-2">
          ...
          <ActivityCard title="Recent bookings" empty="No bookings yet">
            {activity.bookings.map((b) => (
              <Link key={b.id} to="/admin/bookings" className="flex items-center justify-between py-2 hover:bg-cream-warm rounded-lg px-2 -mx-2">
                <div className="min-w-0">
          ...
          <ActivityCard title="Recent forum posts" empty="No posts yet">
            {activity.posts.map((p) => (
              <Link key={p.id} to={`/forum/${p.id}`} className="flex items-center justify-between py-2 hover:bg-cream-warm rounded-lg px-2 -mx-2">
                <div className="min-w-0">
...
function SectionCard({ to, icon: Icon, title, desc, color }) {
  return (
    <Link to={to} className="card-soft p-6 hover:shadow-2xl hover:-translate-y-1 transition group">
...
function ActivityCard({ title, children, empty }) {
  ...
  return (
    <div className="card-soft p-5">
      ...
      {hasItems ? <div className="divide-y divide-ink/5">{children}</div> : (
