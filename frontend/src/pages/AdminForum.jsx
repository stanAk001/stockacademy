...
function ContentRow({ kind, item, onModerate }) {
  return (
    <div className={`card-soft p-4 ${item.is_removed ? 'opacity-60 border-2 border-bear-500/20' : ''}`}>
      ...
      {kind === 'post' && <p className="font-display font-bold text-base mb-1"><Link to={`/forum/${item.id}`} className="hover:underline">{item.title}</Link></p>}
      {kind === 'comment' && <p className="text-xs text-ink/55 mb-1">on: {item.post_title}</p>}
      <p className="text-sm text-ink/75 line-clamp-3 whitespace-pre-wrap">{item.content}</p>
