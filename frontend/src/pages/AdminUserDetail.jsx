...
export default function AdminUserDetail() {
    ...
    return (
      <Layout>
        <div className="max-w-4xl mx-auto p-10">
          <div className="h-32 bg-ink/5 rounded-3xl animate-pulse" />
        </div>
      </Layout>
    );
  }
  ...
  if (!data) {
    return (
      <Layout>
        <div className="p-10 text-center text-ink/60">User not found</div>
      </Layout>
    );
  }
          ...
          {parseInt(id) !== me.id && (
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-ink/5">
              {user.plan === 'premium' ? (
                ...
                <button onClick={() => act('grant_premium')} className="btn-primary text-sm bg-gradient-to-r from-coral-400 to-sun-400 text-ink">
                  <Sparkles size={14} />Grant premium
                </button>
              )}
              {user.is_banned ? (
                <button onClick={() => act('unban')} className="px-4 py-2 rounded-full bg-bull-100 text-bull-700 text-sm font-bold">
                  <UserCheck size={14} className="inline mr-1" />Unban
                </button>
              ) : !user.is_admin && (
                <button
                  onClick={() => {
                  ...
                  className="px-4 py-2 rounded-full bg-bear-500 text-white text-sm font-bold"
                >
            ...
            {posts.length === 0 ? <Empty /> : posts.map((p) => (
              <div key={p.id} className="py-2 border-b border-ink/5 last:border-0 text-sm">
                ...
                <p className="text-xs text-ink/55">
                  ...
                  {p.is_removed && <span className="text-bear-500"> · removed</span>}
                </p>
              </div>
            ))}
          </Section>
...
function Empty() {
  return <p className="text-sm text-ink/45 italic py-2">Nothing yet</p>;
}