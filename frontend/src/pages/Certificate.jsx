...
export default function Certificate() {
  ...
  useEffect(() => {
    load();
    api.get('/geo').then(({ data }) => data.success && setGeo(data)).catch(() => {});
  }, []);

  // Geo decides display currency: Nigeria → ₦, everywhere else → $.
  const isForeign = Boolean(geo?.country && geo.country !== 'NG');
  const priceLabel = isForeign
    ? `$${Math.round((data?.pricing?.price_usd_cents || 1500) / 100)}`
    : `₦${(data?.pricing?.price_ngn || 4000).toLocaleString()}`;
  ...
  const getCertificate = async () => {
    ...
    setPaying(true);
    try {
      const { data } = await api.post('/certificates/initialize', { country: geo?.country || 'NG' });
      if (data.success) {
        if (data.free) {
      ...
      <Layout>
        <div className="max-w-3xl mx-auto px-3 sm:px-6 py-8 sm:py-12">
          <div className="text-center mb-8">
            ...
            <p className="text-ink/60 mt-3 text-sm sm:text-lg">Your StockAcademia certificate is ready.</p>
          </div>
    ...
    <Layout>
      <div className="max-w-3xl mx-auto px-3 sm:px-6 py-8 sm:py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-ink/60 mt-3 text-sm sm:text-lg">Time to claim your StockAcademia Graduate certificate.</p>
        </div>
          ...
          <div className="relative">
            ...
            <div className="bg-cream/5 border border-cream/10 rounded-xl p-4">
              ...
              {data.pricing.is_premium ? (
                ...
                <p className="font-display text-3xl font-black">{priceLabel}</p>
              )}
              {!data.pricing.is_premium && (
        ...
        <div className="text-center">
          ...
          >
            {paying ? (
              ...
              <><Award size={18} /> Get certificate — {priceLabel}</>
            )}
          </button>
