      ...
      <div className="relative grid lg:grid-cols-12 gap-6 items-center">
        ...
        <div className="lg:col-span-7">
          ...
          <h2 className="font-display text-2xl sm:text-4xl font-black leading-[1.05] mb-2">
            Search any stock.<br/>
            <span className="italic text-sun-300">Get a full analysis report.</span>
          </h2>
          <p className="text-cream/70 text-sm sm:text-base mb-5 max-w-md leading-relaxed">
            Quality, value, momentum, and risk scores on any US or Nigerian stock. Read it, then practice with it in the simulator.
          </p>
          ...
          <div className="flex flex-wrap gap-2">
            <Link to="/rankings" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-sun-300 text-ink text-xs font-bold hover:bg-sun-400 transition">
              <BarChart3 size={12}/> Browse top-rated stocks
            </Link>
        ...
        <div className="lg:col-span-5 hidden lg:block">
          <div className="bg-cream/5 backdrop-blur-sm border border-cream/10 rounded-2xl p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="font-mono text-sm font-bold">EXAMPLE</p>
                <p className="text-xs text-cream/50">What a report looks like</p>
              </div>
              <div className="text-right">
                <p className="font-display text-4xl font-black leading-none">78</p>
                <p className="text-[10px] text-cream/50 font-bold uppercase">/ 100</p>
              </div>
            </div>
            <div className="space-y-2">
              {[
                { label: 'Quality', score: 84, color: 'bg-bull-400' },
                { label: 'Value', score: 62, color: 'bg-sun-300' },
                { label: 'Momentum', score: 71, color: 'bg-coral-400' },
                { label: 'Risk safety', score: 80, color: 'bg-sage-400' },
              ].map((f) => (
                <div key={f.label} className="flex items-center gap-2">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-cream/60 w-20">{f.label}</span>
                  ...
                  <span className="text-[10px] font-mono font-bold w-6 text-right">{f.score}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-cream/60 italic mt-3 pt-3 border-t border-cream/10">
              "Quality business at a fair price — a classic compounder setup."
            </p>
          </div>
        </div>
      </div>
