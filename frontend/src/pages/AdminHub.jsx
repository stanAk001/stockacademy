...
import {
  ...
  Wallet, BookOpen, Sparkles, ArrowRight, TrendingUp, ClipboardCheck, Send,
  Newspaper, Loader2, ExternalLink,
} from 'lucide-react';
...
export default function AdminHub() {
  ...
  const [recapDone, setRecapDone] = useState(null); // { slug, title } after a successful run

  const generateRecap = async () => {
    setRecapBusy(true);
    try {
      const { data } = await api.post('/admin/generate-recap');
      setRecapDone({ slug: data.slug, title: data.title });
      toast.success('Today\'s recap is live on /insights.');
    } catch (err) {
      // The controller returns a clear reason (ai_not_configured / ai_error / parse).
      toast.error(err.response?.data?.message || 'Could not generate the recap.');
    } finally {
      setRecapBusy(false);
    }
  };
    ...
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          ...
          />
          <SectionCard
            to="/admin/portfolio-reviews"
            icon={ClipboardCheck}
            title="Portfolio reviews"
            desc="Reply to premium members' portfolio review requests"
            color="from-sun-300 to-coral-400"
          />
          <SectionCard
            to="/admin/broadcast"
            icon={Send}
            title="Telegram broadcast"
            desc="Message all premium members with a linked Telegram"
            color="from-bull-400 to-sage-400"
          />
        </div>

        {/* Content engine — generate the daily market recap on demand */}
        <h2 className="font-display text-2xl font-bold mb-4">Content engine</h2>
        <div className="card-dark p-6 mb-10">
          <div className="flex flex-col sm:flex-row sm:items-center gap-5">
            <div className="w-12 h-12 rounded-2xl bg-sun-300 text-ink grid place-items-center shrink-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              {recapDone && (
                <Link
                  to="/insights"
                  className="inline-flex items-center gap-1.5 text-sm font-semibold text-sun-300 mt-2 hover:underline"
                >
                  View "{recapDone.title}" <ExternalLink size={14} />
                </Link>
              )}
            </div>
            <button
            ...
            >
              {recapBusy ? <Loader2 size={18} className="animate-spin" /> : <Newspaper size={18} />}
              {recapBusy ? 'Generating…' : 'Generate today\'s recap'}
            </button>
          </div>
        </div>
